...
import { useGetUsersQuery, useUserPasswordChangeMutation } from './UserApi';
import { useTranslation } from 'react-i18next';
import ClearIcon from '@mui/icons-material/Clear';
import { useDeleteUserMutation } from 'views/pages/MemberManagement/User/UserApi';
import { UserValidate } from './UserValidatePassword';
...
const UserListTable = () => {
    ...
    const { data, isLoading } = useGetUsersQuery(new URLSearchParams({ ...paginationInfo, ...searchCondition }).toString());
    ...
    const handleClick = (row) => {
        navigate(URL.MEMBER_USER_DETAIL + `/${row.seq}`);
    };
    ...
    const [onDelete, deleteResult] = useDeleteUserMutation();
    ...
    useEffect(() => {
        if (deleteResult.data) {
            ...
            if (deleteResult.data.status === 'success' || deleteResult.data.status === 200) {
                setShow(false);
                setShowDelete(false);
                URL.MEMBER_USER && navigate(URL.MEMBER_USER);
            }
        }
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [deleteResult, navigate, t, URL.MEMBER_USER]);
    ...
    return (
        <>
            ...
            <Grid style={{ textAlign: 'end' }}>
                <CreateButton to={URL.MEMBER_USER_CREATE} />
            </Grid>
            <Modal open={show} onClose={showModalHide} aria-labelledby="modal-modal-title" aria-describedby="modal-modal-description">
                <Box sx={{ ...style, width: 800 }}>
                    <Form validateSchema={UserValidate} setOpen={setShow} mutation={useUserPasswordChangeMutation}>
            ...
            <DeleteConfirm show={showDelete} setShow={showModalDelete} onDelete={onDelete} code={code} />
        </>
    );
};
