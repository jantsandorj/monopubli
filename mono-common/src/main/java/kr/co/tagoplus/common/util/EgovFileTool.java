...
import kr.co.tagoplus.common.component.EgovProperties;
import kr.co.tagoplus.common.component.Globals;
	...
	public static Vector<List<String>> parsFileByChar(String parFile, String parChar, int parField) throws Exception {
		...
		try {
			...
			if (file.exists() && file.isFile()) {
				...
				String[] strArr = TextUtil.split(strBuff.toString(), parChar);
			...
			if (targetF.exists() && targetF.isDirectory()) {
				...
				if (logF.exists()) {
					...
					if (lastStr.equals("END")) {
						EgovFileMntrg t = new EgovFileMntrg(WebUtil.filePathBlackList(targetDirPath), logF);
						t.start();
					}
				} else {
