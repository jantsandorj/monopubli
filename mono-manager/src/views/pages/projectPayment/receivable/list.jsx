...
// Api
import { useGetProjectReceivableListPaymentsQuery} from 'api/project/projectPayment';
...
const ReceivableList = () => {
    ...
    const { data, isLoading, isFetching } = useGetProjectReceivableListPaymentsQuery(new URLSearchParams({ ...options }).toString());
    ...
    const handleRowContent = (type, row) => {
        ...
        if(currentItem != null){
            switch (type) {
                ...
                case 'price':
                    return (
                        <>
                            ...
                            {(currentItem.actualDate != null && currentItem.actualDate != '') ? currentItem.actualDate : (
                ...
                case 'status':
                    return currentItem.paymentStatus == 'P' ? (
                        <Typography sx={{color: '#f53334'}}>
                            {t('project_payment_status_processing')}
                        </Typography>
                    ) : (
                        <Typography>
                            {t('project_payment_status_complete')}
                        </Typography>
                    );
                case 'date':
                    return currentItem.estimatedDate;
                default:
                    return findCorName(currentItem.paymentType, i18n.language === 'kr' ? 'codeNm' : 'codeEngNm', projectPaymentTypeData?.data);
            }
        }
    ...
    return (
        <MainCard title={t('project_payment_receivable_list')}>
            {isLoading && <Loader />}
            <Stats type={"receivable"}/>
            {/*FILTER*/}
            <CustomFilter
            ...
            <Grid marginTop={2}>
                <DataTable
                    ...
                    columns={[
                        ...
                        {
                            name: t('project_payment_estimated_date'),
                            selector: (row) => handleRowContent('date', row),
                            center: true,
                            width: '15%'
                        },
                        {
